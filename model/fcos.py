...
class FCOS(nn.Module):
    ...
    def forward(self,x):
        ...
        mask_preds  list contains five [batch_size,1,h,w]
        ''' 
        C2,C3,C4,C5=self.backbone(x)
        all_P=self.fpn([C3,C4,C5])
        cls_logits,cnt_logits,reg_preds=self.head(all_P)
        seg_preds=self.seg_head(all_P[:3], C2)

        return [cls_logits,cnt_logits,reg_preds], seg_preds
...
class MaskDetecHead(nn.Module):
    def __init__(self):
        super(MaskDetecHead, self).__init__()
        self.softmax = nn.Softmax(dim=1)
    def forward(self,input_mask):
        '''
        input_mask:[[batch,5,h,w]]
        '''
        for batch in range(input_mask[0].shape[0]):
            masks_softmax = self.softmax(input_mask[0])
        return masks_softmax
...
class FCOSDetector(nn.Module):
    def __init__(self,mode="training",config=None):
        ...
        self.mask_detection_head = MaskDetecHead()
    
    def forward(self,inputs):
        '''
        inputs 
        [training] list  batch_imgs,batch_boxes,batch_classes, batch_masks
        [inference] img
        '''

        if self.mode=="training":
            batch_imgs,batch_boxes,batch_classes, batch_masks=inputs
            out, pre_masks=self.fcos_body(batch_imgs)
            targets=self.target_layer([out,batch_boxes,batch_classes])
            losses=self.loss_layer([out,targets])
            loss_mask = self.mask_loss(pre_masks, batch_masks)
            # 
            ...
            masks = self.mask_detection_head(pre_masks)
