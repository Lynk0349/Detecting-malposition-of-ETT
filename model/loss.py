...
def compute_seg_loss(preds,targets):
    '''
    Args  
    preds: [[batch_size,class_num,_h,_w]]
    targets: [batch_size,1,_h,_w]
    '''
    batch_size=targets.shape[0]
    preds_reshape=[]
    class_num=preds[0].shape[1]
    ...
    for pred in preds:
        pred=pred.permute(0,2,3,1) #[batch, h, w, class_num]
        pred=torch.reshape(pred,[batch_size,-1,class_num]) #[batch, h*w, class_num]
        preds_reshape.append(pred)

    targets=targets.permute(0,2,3,1) #[batch, h, w, 1]
    targets=torch.reshape(targets,[batch_size,-1]) #[batch, h*w*1]
    targets_one_hot = F.one_hot(targets, num_classes = 5) #[batch, h*w, 5]
    
    preds=torch.cat(preds_reshape,dim=1) #[batch_size, h*w, class_num]
    assert preds.shape[:2]==targets.shape[:2]
    loss=[]
    for batch_index in range(batch_size):
        pred_pos_ce=preds[batch_index] #[h*w, class_num]
        pred_pos_dc_ETT=preds[batch_index,:,1]
        pred_pos_dc_branch=preds[batch_index,:,3]
        target_pos=targets[batch_index] #[h*w]
        
        target_pos_ETT = targets_one_hot[batch_index,:,1]
        target_pos_branch = targets_one_hot[batch_index,:,3]
        loss_dc = (DiceLoss()(pred_pos_dc_ETT, target_pos_ETT) + DiceLoss()(pred_pos_dc_branch, target_pos_branch))
        loss_ce = CrossEntropyLoss()(pred_pos_ce, target_pos)
        loss.append(loss_dc+loss_ce)
...
class Mask_loss(nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self, in_mask, mask_gt):
        mask_loss = compute_seg_loss(in_mask, mask_gt)
