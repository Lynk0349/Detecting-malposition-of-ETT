...
class Transforms(object):
    ...
    def __call__(self, img, boxes, masks):
        if random.random() < 0.3:
            img, boxes, masks = colorJitter(img, boxes, masks)
        if random.random() < 0.5:
            img, boxes, masks = random_rotation(img, boxes, masks)
        if random.random() < 0.5:
            img, boxes, masks = random_crop_resize(img, boxes, masks)
        return img, boxes, masks

def colorJitter(img, boxes, masks, brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1):
...
def random_rotation(img, boxes, masks, degree=10):
    d = random.uniform(-degree, degree)
    w, h = img.size
    rx0, ry0 = w / 2.0, h / 2.0
    img = img.rotate(d)
    # 
    masks = masks.rotate(d)
    # 
    a = -d / 180.0 * math.pi
...
def random_crop_resize(img, boxes, masks, crop_scale_min=0.2, aspect_ratio=[3./4, 4./3], remain_min=0.7, attempt_max=10):
    success = False
    boxes = torch.from_numpy(boxes)
